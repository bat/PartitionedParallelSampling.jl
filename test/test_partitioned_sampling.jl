...
# BAT experimental:
using BAT: bat_compare

# ToDo: Use faster integrator to test
using AHMI: AHMIntegration

using PartitionedParallelSampling: convert_to_posterior


@testset "partitioned_sampling" begin
    ...
    ahmi = AHMIntegration(whitening = NoWhitening(), max_startingIDs = 10^3)
    mcmc_exp = MCMCSampling(mcalg = MetropolisHastings(), nsteps = 400, nchains=4, strict=false)

    ps = PartitionedSampling(sampler = mcmc, npartitions=4, exploration_sampler=mcmc_exp, integrator = AHMIntegration(), nmax_resampling=5);
